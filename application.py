...
import requests
app = Flask(__name__)
app.secret_key = 'CRNVK5NJE_NIRYJ1wulsigde'

CLIENT_ID = json.loads(open('client_secrets.json', 'r').read())['web'
                                                                ]['client_id']
APPLICATION_NAME = 'Technical Skills Catalog'
...
def categoryItemsJSON(category_id):
    ...
    itemCategories = \
        session.query(ItemCategory).filter_by(category_id=category_id).all()
...
def newCategory():
    ...
    if request.method == 'POST':
        ...
        user_id = login_session['user_id']
        return render_template('newcategory.html', items=items,
                               user_id=user_id)
...
def editCategory(category_id):
    ...
    if categoryToEdit.user_id != login_session['user_id']:
        flash('Only the creator of this category can delete it.'
              )
        return redirect('/')
    itemCategoryIds = \
...
def deleteCategory(category_id):
    ...
    if request.method == 'POST':
        ...
        for x in itemCategoriesToDelete:
            session.delete(x)
        session.commit()
        return redirect('/')
    else:
...
def removeItemFromCategory(category_id, item_id):
    ...
    if itemCategory.category.user_id != login_session['user_id']:
        flash('Only the creator of this category can edit it.'
              )
        return redirect(url_for('displayItemsInCategory',
...
def deleteItem(item_id):
    ...
    if itemToDelete.user_id != login_session['user_id']:
        flash('Only the creator of this item can delete it.'
              )
        return redirect('/')
    if request.method == 'POST':
        itemCategories = \
            session.query(ItemCategory).filter_by(item_id=item_id).all()
